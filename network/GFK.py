...
class GFK:
	def __init__(self, dim = 20, eps = 1e-20):
		'''
		Init func
	...
	def train_pca(self, data, mu_data, std_data, subspace_dim):
		...
		if isinstance(subspace_dim, float):
			cummulated = np.cumsum(variances) / (np.sum(variances) + self.eps)
...
if __name__ == "__main__":
	Xs = np.random.rand(2, 40)
	Xt = np.random.rand(2, 40)
	gfk = GFK(dim = 20)
	G, Xs_new, Xt_new = gfk.fit(Xs, Xt)
